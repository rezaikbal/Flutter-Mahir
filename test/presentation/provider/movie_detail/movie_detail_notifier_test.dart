...
@GenerateMocks([
  DetailMovieUsecase,
  RecommendedMovieUsecase,
  WatchlistMovieStatusUsecase,
...
void main() {
  ...
  setUp((){
    ...
    provider = MovieDetailNotifier(
        detailMovieUsecase: mockGetMovieDetail,
        recommendedMovieUsecase: mockGetMovieRecommendations,
        watchlistMovieUsecase: mockGetWatchListStatus,
  ...
  group('Watchlist', () {
    ...
    test('should update watchlist message when add watchlist failed', () async {
      ///arrange
      when(mockSaveWatchlist.execute(testMovieDetail)).thenAnswer((_) async => const Left(DatabaseFailure('Failed')));
      when(mockGetWatchListStatus.execute(testMovieDetail.id)).thenAnswer((_) async => false);
      ///act
      await provider.addWatchlist(testMovieDetail);
      ///assert
  ...
  group('Get Movie Recommendations', () {
    ...
    test('should update recommendation state when data is gotten successfully', () async {
      ...
      await provider.fetchMovieDetail(tId);
      ///assert
      expect(provider.recommenddationsState, RequestState.Success);
      expect(provider.movieRecommendations, tMovies);
    });
    ...
    test('should update error message when request in successful', () async {
      ...
      await provider.fetchMovieDetail(tId);
      ///assert
      expect(provider.recommenddationsState, RequestState.Error);
